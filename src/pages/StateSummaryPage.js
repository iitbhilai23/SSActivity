...
function StateSummaryPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        const response = await interceptor.get("/verification-summary/state-summary");
        ...
        toast.error("Failed to fetch state summary");
      }
    };
    fetchSummary();
  }, []);
  ...
  const verifiedSchools = summary.filter(s => s.verified > 0).length;
  ...
  return (
    <div className="summary-container">
      <h2 className="summary-title">📊 State Summary Dashboard</h2>
...
export default StateSummaryPage;