...
function DistrictSummaryPage() {
    ...
    const fetchSummary = async () => {
      try {
        const response = await interceptor.get(
          `/verification-summary/district-summary/${user.district_id}`
        );
        ...
        toast.error("Failed to fetch district summary");
      }
    };
    if (user?.role === "DEO") fetchSummary();
  }, [user]);
  ...
  return (
    <div className="summary-container">
      <h2 className="summary-title">📊 District Summary Dashboard</h2>
